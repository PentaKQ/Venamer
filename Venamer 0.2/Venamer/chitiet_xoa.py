...
class Ui_Form_xoa(QDialog):
    ...
    def loaddata(self, Form, files, kitucanxoa, gioihan, count, xoa_gioihan):
        row = 0
            ...
            if xoa_gioihan == True:
                if file[len(file)-len(gioihan):len(file)] == str(gioihan):
                    if file.find(str(kitucanxoa)) != -1:
                        self.tableWidget.setItem(row, 1, QtWidgets.QTableWidgetItem(file.replace(str(kitucanxoa),'')))
                    else:
